...
for i in 1:D
    ...
    for j in 1:D
        for k in j:D
            ...
            return_function = :(0.5*($(ξᵃ_k)(args[1:$D]...)*$(g.uu[i,j]) + $(ξᵃ_j)(args[1:$D]...)*$(g.uu[i,k])))
...
for i in 1:D
    ...
    for j in 1:D
        for k in j:D
            ...
            return_function = :(0.5*($(g.uu[i,j])*$(lᵃ_k) + $(g.uu[i,k])*$(lᵃ_j) + $(ξᵃᵇ_ij)*$(ξᵃ_k) + $(ξᵃᵇ_ik)*$(ξᵃ_j)))
...
#Mᵃᵇ = ξᵃᵉξᵇ_e = ξᵃᵉξᵇᶠg_ef

#∂Mᵃᵇ/∂ξ_cd = gᵃᶜξᵇᵈ + gᵃᵈξᵇᶜ
#∂Mᵃᵇ/∂ξ_cd - (1/D)∂Mᵃᵇ/∂ξ_ef*g_ef*g_ᶜᵈ= gᵃᶜξᵇᵈ + gᵃᵈξᵇᶜ - (2/D)ξᵃᵇgᶜᵈ
...
for i in 1:D
    for j in i:D
        ...
        for k in 1:D
            for l in k:D
                m = m+1
                return_expression = :(($(g.uu[i,k])*$(ξᵃᵇ_fun_symbols[j,l])(args...)) + $(g.uu[i,l])*$(ξᵃᵇ_fun_symbols[j,k])(args...))
                (debug == true) && println(return_expression)
                @eval Symbolics.derivative(::typeof($(Mᵃᵇ_fun_symbols[i,j])), args::NTuple{$(D*(D+1)÷2), Any}, ::Val{$m}) = $return_expression
            end
        end
    end
end
